...
public class SwipeLayout extends FrameLayout {
	...
	private Map<View, Rect> mViewBoundCache = new HashMap<>();// save all
																// children's
																// bound,
																// restore in
																// onLayout
	...
	public SwipeLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		int dragEdgeChoices = a.getInt(R.styleable.SwipeLayout_drag_edge,
				DRAG_RIGHT);
		mEdgeSwipesOffset[DragEdge.Left.ordinal()] = a.getDimension(
				R.styleable.SwipeLayout_leftEdgeSwipeOffset, 0);
		mEdgeSwipesOffset[DragEdge.Right.ordinal()] = a.getDimension(
				R.styleable.SwipeLayout_rightEdgeSwipeOffset, 0);
		mEdgeSwipesOffset[DragEdge.Top.ordinal()] = a.getDimension(
				R.styleable.SwipeLayout_topEdgeSwipeOffset, 0);
		mEdgeSwipesOffset[DragEdge.Bottom.ordinal()] = a.getDimension(
				R.styleable.SwipeLayout_bottomEdgeSwipeOffset, 0);
		setClickToClose(a.getBoolean(R.styleable.SwipeLayout_clickToClose,
				mClickToClose));
		...
		int ordinal = a.getInt(R.styleable.SwipeLayout_show_mode,
	...
	private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
		...
		public int clampViewPositionVertical(View child, int top, int dy) {
			if (child == getSurfaceView()) {
				...
				int surfaceViewTop = surfaceView == null ? 0 : surfaceView
						.getTop();
				switch (mCurrentDragEdge) {
				case Left:
				case Right:
		...
		public boolean tryCaptureView(View child, int pointerId) {
			boolean result = child == getSurfaceView()
					|| getBottomViews().contains(child);
			if (result) {
		...
		public void onViewPositionChanged(View changedView, int left, int top,
			...
			int evLeft = surfaceView.getLeft(), evRight = surfaceView
					.getRight(), evTop = surfaceView.getTop(), evBottom = surfaceView
					.getBottom();
			if (changedView == surfaceView) {
				...
				if (mShowMode == ShowMode.PullOut) {
					...
					if (currentBottomView != null) {
						currentBottomView.layout(rect.left, rect.top,
								rect.right, rect.bottom);
					}

					int newLeft = surfaceView.getLeft() + dx, newTop = surfaceView
							.getTop() + dy;
					...
					surfaceView.layout(newLeft, newTop, newLeft
							+ getMeasuredWidth(), newTop + getMeasuredHeight());
				}
			}
	...
	private void safeBottomView() {
		...
		if (status == Status.Close) {
			...
			View currentBottomView = getCurrentBottomView();
			if (currentBottomView != null
					&& currentBottomView.getVisibility() != VISIBLE) {
	...
	protected void dispatchRevealEvent(final int surfaceLeft,
		...
		for (Map.Entry<View, ArrayList<OnRevealListener>> entry : mRevealListeners
			...
			if (isViewShowing(child, rect, mCurrentDragEdge, surfaceLeft,
				...
				for (OnRevealListener l : entry.getValue()) {
					l.onReveal(child, mCurrentDragEdge, Math.abs(fraction),
							distance);
			...
			if (isViewTotallyFirstShowed(child, rect, mCurrentDragEdge,
					surfaceLeft, surfaceTop, surfaceRight, surfaceBottom)) {
				mShowEntirely.put(child, true);
				for (OnRevealListener l : entry.getValue()) {
					if (mCurrentDragEdge == DragEdge.Left
							|| mCurrentDragEdge == DragEdge.Right)
						...
						l.onReveal(child, mCurrentDragEdge, 1,
								child.getHeight());
				}
			}
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		...
		try {
			gravity = (Integer) params.getClass().getField("gravity")
					.get(params);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (gravity > 0) {
			gravity = GravityCompat.getAbsoluteGravity(gravity,
	...
	void layoutPullOut() {
		...
		if (surfaceView != null) {
			surfaceView.layout(surfaceRect.left, surfaceRect.top,
		...
		if (bottomViewRect == null)
			bottomViewRect = computeBottomLayoutAreaViaSurface(
					ShowMode.PullOut, surfaceRect);
		if (currentBottomView != null) {
			currentBottomView.layout(bottomViewRect.left, bottomViewRect.top,
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		return super.onTouchEvent(event) || mIsBeingDragged
	...
	/***
	 * Returns the percentage of revealing at which the view below should the
	 * view finish opening if it was already open before dragging
	 * 
	 * @returns The percentage of view revealed to trigger, default value is
	 *          0.25
	 */
	...
	/***
	 * Allows to stablish at what percentage of revealing the view below should
	 * the view finish opening if it was already open before dragging
	 * 
	 * @param willOpenPercentAfterOpen
	 *            The percentage of view revealed to trigger, default value is
	 *            0.25
	 */
	...
	/***
	 * Returns the percentage of revealing at which the view below should the
	 * view finish opening if it was already closed before dragging
	 * 
	 * @returns The percentage of view revealed to trigger, default value is
	 *          0.25
	 */
	...
	/***
	 * Allows to stablish at what percentage of revealing the view below should
	 * the view finish opening if it was already closed before dragging
	 * 
	 * @param willOpenPercentAfterClose
	 *            The percentage of view revealed to trigger, default value is
	 *            0.75
	 */
	...
	private boolean performAdapterViewItemLongClick() {
		...
		if (t instanceof AdapterView) {
			...
			try {
				Method m = AbsListView.class.getDeclaredMethod(
						"performLongPress", View.class, int.class, long.class);
	...
	class SwipeDetector extends GestureDetector.SimpleOnGestureListener {
		...
		public boolean onDoubleTap(MotionEvent e) {
			if (mDoubleClickListener != null) {
				...
				if (bottom != null && e.getX() > bottom.getLeft()
	 ...
	 */
	protected void processHandRelease(float xvel, float yvel,
		...
		if (currentDragEdge == DragEdge.Left) {
			if (xvel > minVelocity)
				...
				float openPercent = 1f * getSurfaceView().getLeft()
						/ mDragDistance;
			...
			if (yvel > minVelocity)
				...
				float openPercent = 1f * getSurfaceView().getTop()
						/ mDragDistance;
				if (openPercent > willOpenPercent)
					open();
				else
	...
	public void open(boolean smooth, boolean notify) {
		...
		if (smooth) {
			...
			if (getShowMode() == ShowMode.PullOut) {
				...
				if (bottom != null) {
					bottom.layout(bRect.left, bRect.top, bRect.right,
							bRect.bottom);
				}
			}
			if (notify) {
	...
	protected void onViewRemoved(View child) {
		for (Map.Entry<DragEdge, View> entry : new HashMap<DragEdge, View>(
				mDragEdges).entrySet()) {
	 ...
	 */
	@Deprecated
	public void setBottomViewIds(int leftId, int rightId, int topId,
			int bottomId) {
	...
	private void updateBottomViews() {
		...
		if (currentBottomView != null) {
			if (mCurrentDragEdge == DragEdge.Left
					|| mCurrentDragEdge == DragEdge.Right) {
				mDragDistance = currentBottomView.getMeasuredWidth()
